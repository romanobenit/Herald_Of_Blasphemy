...
export default class Boss02 extends Phaser.Scene {
  ...
  private _evolutionPhase: number = 1;
  ...
  create() {
    ...
    this._boss = new Player(
      { scene: this, x: 100, y: 100, key: "melemia" },
      "Melemia", 600, 600, 100, 100,
  ...
  private _drawPlayerHealthBar(): void {
    ...
    if (ratio > 0.5) {
      ...
      r = Math.floor(170 * (1 - t)); g = Math.floor(170 * t); b = 0;
    } else if (ratio > 0.25) {
      const t = (ratio - 0.25) / 0.25;
      r = Math.floor(70 * t + 100); g = 0; b = 0;
    } else {
      const t = ratio / 0.25;
      r = Math.floor(70 * t + 30); g = 0; b = 0;
    }
    ...
    this._playerHealthBar
      .lineStyle(4, 0x000000, 1).strokeRoundedRect(550, 510, maxWidth, 20, 10)
      .setDepth(1000);
  }
  ...
  private _startEvolutionGlow(): void {
    ...
    this._evolutionGlowTween = this.add.tween({
      ...
      alpha: 0.2, yoyo: true, repeat: -1, duration: 500, ease: "Sine.easeInOut",
    });
  }
...
private _showEvolutionPopup(): void {
  ...
  private _doEvolution(newPhase: number): void {
                ...
                this.add.tween({
                  ...
                  onComplete: () => {
                    this.time.delayedCall(1200, () => {
                      flash.destroy(); evoText.destroy(); faseText.destroy();
    ...
    this.add.tween({ targets: rays, angle: 180, duration: 1200, ease: "linear" });
  }
