...
export default class Preloader extends Phaser.Scene {
  ...
  preload() {
    ...
    // Carica il video della cinematica intro
    this.load.video("introVideo", "assets/IntroProgetto.mp4", true);

    this.loadAssets();
  }

  create() {
    let _sfondoNero: Phaser.GameObjects.Image = this.add
      .image(0, 0, "sfondoNero")
      .setOrigin(0, 0).setAlpha(0).setDepth(1002);

    this.add
      .image(this.game.canvas.width / 2, this.game.canvas.height / 2, "PreImage")
      .setOrigin(0.5).setScale(5);

    this._mita = this.add
      .text(this.game.canvas.width / 2, this.game.canvas.height / 2 + 100, "Embrace your faith.")
      ...
      .on("pointerdown", () => {
        // Sblocca il contesto audio nel momento esatto del click utente
